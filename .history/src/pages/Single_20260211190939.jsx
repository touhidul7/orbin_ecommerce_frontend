 ...
 *
 * ⚠️ IMPORTANT:
 * - Backend response shape supported: { message: "...", "0": [ ... ] } OR plain arrays
 */
...
function pickArrayFromApi(payload) {
  ...
  // sometimes nested like {0: [[...]]}
  if (Array.isArray(payload?.["0"]?.[0])) return payload["0"][0];
...
const Single = () => {
  ...
  // ✅ keep your existing context usage
  ...
  const loadRelatedProducts = useCallback(
    async (select_category, currentProductId, signal) => {
      ...
      try {
        ...
        const list = pickArrayFromApi(payload);
        const filteredProducts = (list || []).filter(
          (product) => Number(product.id) !== Number(currentProductId)
        );

        setRelatedProducts(filteredProducts);
      } catch (error) {
        if (error?.name !== "AbortError") console.error("Error fetching related products:", error);
      } finally {
  ...
  const loadData = useCallback(
    async (signal) => {
      ...
      try {
        ...
        // ✅ FIX: supports { "0": [ {..} ] } and arrays
        const arr = pickArrayFromApi(payload);
        const product = arr?.[0] || null;

        setData(product);

        if (product?.select_category) {
          loadRelatedProducts(product.select_category, product.id, signal);
        }
      } catch (error) {
  ...
  // ✅ Recommended list from CURRENT PRODUCT (for every product)
  const recommendedList = useMemo(() => {
    if (!data) return [];
    const recs = normalizeRecommendedProducts(data, { depth: 1 });

    // exclude current product itself; keep others (even if in cart)
    return recs.filter((p) => {
      const rid = Number(p?.id);
      if (!rid) return false;
      return Number(data?.id) !== rid;
    });
  }, [data]);
  ...
  const toggleRecommended = (product, checked) => {
    ...
    setCart?.(next); // ✅ update context state instantly
  };

  // Handlers for cart actions (UNCHANGED)
      ...
      {loading ? (
        ...
        <div className="mt-20 px-4">
          <div className="max-w-xl mx-auto bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-gray-600 mb-4">Go back</p>
            <Link
              to="/"
              className="inline-flex items-center justify-center bg-black text-white font-bold py-2 px-4 rounded-md hover:bg-[#313131] transition"
            >
              Back to Home
            </Link>
          </div>
